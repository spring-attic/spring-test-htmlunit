...
----
dependencies {
  testCompile 'org.springframework:spring-test-htmlunit:{project-version}'
...
NOTE: The first path segment, `/mail`, after the host is treated as the context root. Alternatively, we could have passed in an optional second argument to `MockMvcWebConnection` to specify a context root of "".
...
Once we have the <<updating-dependencies,correct dependencies>>, we can use Geb in our unit tests. The complete code sample for using Geb and Spring MVC Test can be found in https://github.com/spring-projects/spring-test-htmlunit/blob/master/mail-webapp/src/test/groovy/sample/geb/GebCreateMessagesSpec.groovy[GebCreateMessagesSpec].
...
The following provides information on setting up a development environment that can run https://github.com/spring-projects/spring-test-htmlunit/tree/master/mail-webapp[the sample] in http://www.springsource.org/sts[Spring Tool Suite 3.0.0]. Other IDE's should work using Gradle's IDE support, but have not been tested.
